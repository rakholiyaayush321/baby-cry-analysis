...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (path.includes("monitor.html")) {
    requireAuth(initMonitoringPage);
  } else if (path.includes("dashboard.html")) {
    requireAuth(initDashboardPage);
...
function requireAuth(callback) {
    ...
    if (!token) {
        ...
        // Optionally verify token via backend here, but for now we trust presence
...
function setupAuthUI() {
    ...
    if (token && navLinksContainer) {
        ...
        // Remove login link if it exists and we're not on auth pages
        const loginLink = document.querySelector('a[href="login.html"]');
        if (loginLink && !window.location.pathname.includes("login") && !window.location.pathname.includes("register")) {
